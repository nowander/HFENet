...
import numpy as np
import pandas as pd
from backbone.model import resnet34
# from backbone.model import resnet34
...
#     def __init__(self,inchannel,is_more,is_channel,dilation_level=[1,2,3]):
...
class merger5(nn.Module):
    def __init__(self,inchannel,is_more,is_channel,dilation_level=[1,2,3]):
        ...
        self.branches1 = nn.ModuleList([
            ...
            nn.Conv2d(inchannel, inchannel, stride=1, kernel_size=3, padding=2, dilation=2),
            nn.Conv2d(inchannel, inchannel, stride=1, kernel_size=3, padding=4, dilation=4),
            nn.Conv2d(inchannel, inchannel, stride=1, kernel_size=3, padding=6, dilation=6),
        ])
        # self.upsample2 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.conv1 = nn.Sequential(
            nn.Conv2d(inchannel, inchannel, kernel_size=1, padding=0),
    ...
    def forward(self, *x):
        ...
        branch4 = branch[3].view(b, -1, h)
        dep4 = dep.view(b, -1, h).permute(0, 2, 1)
        out4 = torch.sigmoid(torch.matmul(dep4, branch4))

        out1 = out1 + out4.unsqueeze(1)
        #
        # b, c, h, w = branch[2].size()
        # # [N, C // ratio, H*W]
        # x_phi = branch[2].view(b, c, -1)
        # # [N, H * W, C // ratio]
        # x_theta = branch[2].view(b, c, -1).permute(0, 2, 1).contiguous()
...
class Edge_aware_dialation(nn.Module):
    def __init__(self):
        ...
        self.sa = SpatialAttention()
        #[4, 64, 112, 112])  ([4, 64, 56, 56])  ([4, 128, 28, 28])
        # self.conv1 = nn.Conv2d(128, 64, kernel_size=3, padding=1, stride=1)
        self.conv1 = nn.Sequential(nn.Conv2d(64, 64, kernel_size=3, padding=2, stride=1, dilation=2),
            # nn.Conv2d(128, 64, kernel_size=3, padding=1),
                                 nn.BatchNorm2d(64),
                                 nn.ReLU(inplace=True)
                                 )
        self.conv2 = nn.Sequential(nn.Conv2d(128, 64, kernel_size=3, padding=4, stride=1, dilation=4),
    ...
    def forward(self, x1,x2,x3):
        ...
        x_all_1 = self.conv3(x_all)
        ed = self.sa(x_all_1)
        out = self.up2(self.last_bound(ed))
...
class SFNet(nn.Module):
    def __init__(self):
        ...
        self.rgb_encode = resnet34(pretrained=True)
        self.dep_encode = resnet34(pretrained=True)
        # self.vgg = vgg_left224()
        # self.vgg_depth = vgg_right224()
        self.merge1 = merger5(64, 0,0)  # merger5(64)
        self.merge2 = merger5(64, 1,0)  # merger5(64)
        self.merge3 = merger5(128, 1,1)  # merger3(128)
        ...
        self.merge4 = merger_AM(256,1)  # merge2()
        self.merge5 = merger_AM(512,0)  # merge2()
        # self.merge6 = merger_AM(512,0)

        self.S5 = merge_corr(512,1,1)
        self.S4 = merge_corr(256,1,1)
        self.S3 = merge_corr(128, 1,1)
        self.S2 = merge_corr(64, 2,1)
        ...
        self.conv256_64 = nn.Sequential(nn.Conv2d(256, 64, kernel_size=3, padding=1), nn.BatchNorm2d(64),
                                         nn.ReLU(inplace=True)
                                         )
        self.conv128_64 = nn.Sequential(nn.Conv2d(128, 64, kernel_size=3, padding=1), nn.BatchNorm2d(64),
                                         nn.ReLU(inplace=True)
                                         )
        ...
        self.last_conv3 = nn.Sequential(nn.Conv2d(64, 1, kernel_size=1, padding=0))#, nn.BatchNorm2d(1))
        self.last_conv4 = nn.Sequential(nn.Conv2d(64, 1, kernel_size=1, padding=0))#, nn.BatchNorm2d(1))
    ...
    def forward(self, left, depth):
        A1,A2,A3,A4,A5 = self.rgb_encode(left)
        A1_d, A2_d, A3_d,A4_d, A5_d= self.dep_encode(depth)
        merge1 = self.merge1(A1,A1_d)
        #print('merge1', merge1.shape)#([4, 64, 112, 112])
        # print('A1',A1.shape)
        # print('A2', A2.shape)
        # print('A3', A3.shape)
        # print('A4', A4.shape)
        ...
        merge5 = self.merge5(A5, A5_d)  # ([4, 512, 7, 7])
        # print('merge5', merge5.shape)
        merge4 = self.merge4(A4, A4_d,merge5)  #([4, 256, 14, 14])
        # print('merge4', merge4.shape)
        # print('merge5', merge5.shape)
        # print('merge3', merge3.shape)
        out  = self.S5(merge4,merge5)
        # print('out',out.shape) #[4, 256, 14, 14]
        out1 = self.S4(merge3,out)
        # print('out1',out1.shape)#([4, 128, 28, 28])
        # print('merge3',merge3.shape)
        # print('merge2', merge2.shape)
        out21 = out1 + self.conv256_128(self.up2(out))
        out2 = self.S3(merge2,out21)
        # print('out2', out2.shape) #out2 torch.Size([4, 128, 56, 56])
        # print('merge2', merge2.shape)#merge2 torch.Size([4, 128, 112, 112])
        out22 = out2 + self.conv128_64(self.up2(out1))
        # print('out22', out22.shape)
        # print('merge1', merge1.shape)
        out3 = self.S2(merge1,out22)
        # print('out3', out3.shape)#4, 128, 112, 112
        # print('out', out.shape)
        # print('merge1', merge1.shape)#[4, 64, 224, 224]
        # out33 = out3 + self.conv256_64(self.up8(out))
        # print('out33', out33.shape)
