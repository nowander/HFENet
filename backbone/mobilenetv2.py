...
class MobileNetV2(nn.Module):
    ...
    def forward(self, x):
        ...
        out1 = self.features[2:4](x)
        out2 = self.features[4:7](out1)
        out3 = self.features[7:17](out2)
        out4 = self.features[17:18](out3)
        # x = x.mean([2, 3])
        # x = self.classifier(x)
...
def mobilenet_v2(pretrained=True, progress=True, **kwargs):
    ...
    if pretrained:
        print('loading>>>>>>>>>>')
        pretrained_vgg = load_state_dict_from_url(model_urls['mobilenet_v2'])
        model_dict = {}
