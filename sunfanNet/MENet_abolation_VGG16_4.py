...
class merger_AM(nn.Module):#channel and spatial
    def __init__(self,inchannel,is_more,is_channel):
...
class SFNet(nn.Module):
    def __init__(self):
        ...
        self.merge2 = merger5(128, 1, 1)  # merger5(64)
        self.merge3 = merger5(256, 1, 1)  # merger3(128)
        ...
        self.merge4 = merger_AM(512, 1,0)  # merge2()
        self.merge5 = merger_AM(512, 0,0)  # merge2()
        # self.merge6 = merger_AM(512,0)

        self.S5 = merge_corr(512,2,1)
        self.S4 = merge_corr(512,0,1)
        self.S3 = merge_corr(256, 0,1)
        self.S2 = merge_corr(128, 0,1)
        ...
        self.conv512_256 = nn.Sequential(nn.Conv2d(512, 256, kernel_size=3, padding=1), nn.BatchNorm2d(256),
                                         nn.ReLU(inplace=True)
                                         )
        self.up2 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.up4 = nn.Upsample(scale_factor=4, mode='bilinear', align_corners=True)
        ...
        self.last_conv2 = nn.Sequential(nn.Conv2d(256, 1, kernel_size=1, padding=0))#, nn.BatchNorm2d(1))
        ...
        self.last_conv6 = nn.Sequential(nn.Conv2d(512, 1, kernel_size=1, padding=0))
    def forward(self, left, depth):
        A1, A2, A3, A4, A5 = self.vgg(left)
        A1_d, A2_d, A3_d, A4_d, A5_d = self.vgg_depth(depth)
        ...
        # print('merge3',merge3.shape)
        # print('merge2', merge2.shape)
        out21 = out1 + self.conv512_256(self.up2(out))
        ...
        out22 = out2 + self.conv256_128(self.up2(out1))
        ...
        p0 = self.up8(self.last_conv6(out))
        # print('p0',p0.shape)
        p1 = self.up4(self.last_conv2(out1))
        # print('p1', p1.shape)
        p2 = self.up2(self.last_conv3(out2))
        # print('p2', p2.shape)
        # print('s3',out3.shape)
        p3 = self.last_conv4(out3)
        # print('p3', p3.shape)
